...
#include <mypoints_downsampler/PointsDownsamplerInfo.h>
#include "mypoints_downsampler/ConfigVoxelGridDistanceFilter.h"
...
#define MIN_MEASUREMENT_RANGE 0.0
...
static mypoints_downsampler::PointsDownsamplerInfo points_downsampler_info_msg;
...
static double min_measurement_range = MIN_MEASUREMENT_RANGE;

static void config_callback(const mypoints_downsampler::ConfigVoxelGridDistanceFilter::ConstPtr& input)
{
  voxel_leaf_size = input->voxel_leaf_size;
  measurement_range = input->measurement_range;
  min_measurement_range = input->min_measurement_range;
}
...
static void scan_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
  ...
  if(measurement_range != MAX_MEASUREMENT_RANGE | min_measurement_range != MIN_MEASUREMENT_RANGE){
    scan = removePointsByRange(scan, min_measurement_range, measurement_range);
  }
...
int main(int argc, char** argv)
{
  ...
  private_nh.param<double>("min_measurement_range", min_measurement_range, MIN_MEASUREMENT_RANGE);
  private_nh.param<double>("voxel_leaf_size", voxel_leaf_size, 0.01);

  // Publishers
  filtered_points_pub = nh.advertise<sensor_msgs::PointCloud2>("/filtered_points", 10);
  points_downsampler_info_pub = nh.advertise<mypoints_downsampler::PointsDownsamplerInfo>("/points_downsampler_info", 1000);

  // Subscribers
  ros::Subscriber config_sub = nh.subscribe("/config/voxel_grid_distance_filter", 10, config_callback);
